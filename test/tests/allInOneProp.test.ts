...
it('should work with normal prop', async () => {
  expect(AllInOnePropModel.schema.path('normal')).toBeInstanceOf(Schema.Types.String);
...
it('should work with array prop', async () => {
  expect(AllInOnePropModel.schema.path('array')).toBeInstanceOf(Schema.Types.Array);
  expect((AllInOnePropModel.schema.path('array') as any).caster).toBeInstanceOf(Schema.Types.String);
...
it('should work with map prop', async () => {
  expect(AllInOnePropModel.schema.path('map')).toBeInstanceOf(Schema.Types.Map);
  // tslint:disable-next-line:no-string-literal
  expect(AllInOnePropModel.schema.path('map')['$__schemaType']).toBeInstanceOf(Schema.Types.String);
  const { _id } = await AllInOnePropModel.create({ map: { key1: 'Hello There', key2: 'Hi' } });
  const found = await AllInOnePropModel.findById(_id).orFail().exec();
  expect(Array.from(found.map!)).toEqual([
    ['key1', 'Hello There'],
    ['key2', 'Hi']
  ]);
});
